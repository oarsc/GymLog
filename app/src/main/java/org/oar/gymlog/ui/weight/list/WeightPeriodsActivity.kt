    ...
    override fun onDelayedCreate(savedInstanceState: Bundle?) {
        ...
        WeightPeriodsListHandler(this, internationalSystem).apply {
            ...
            onWeightPeriodClicked {
                if (it.id == Data.weightPeriod?.id) {
                    finish()
                } else {
                    Intent(this@WeightPeriodsActivity, PeriodStatsActivity::class.java).apply {
                        putExtra("weightPeriodId", it.id)
                        startActivityForResult(this, IntentReference.WEIGHT_PERIOD_STATS_DETAILS)
                    }
                }
            }
        }
